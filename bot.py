    ...
    try:
        ...
        try:
            if req.json()["status_code"] == 0:
                ...
                if DebugMode:
                     print(Colorate.Horizontal(Colors.blue_to_purple, f"Sent Share: {SentTotalSentShares} ({impr_id})"))
                else:
                     print(Colorate.Horizontal(Colors.blue_to_purple, f"Sent Share: {SentTotalSentShares} ({impr_id})"))
                     Title(f"Made By Gaming0ndemand / Threads :{str(active_count() - 1)} / Sent Shares :{SentTotalSentShares} / Failed to Send :{TotalFailedReq}")
            else:
                pass
        except:
            TotalFailedReq += 1
...
if __name__ == "__main__":
    Clear()
    itemID = Write.Input("Video Link > ", Colors.blue_to_purple, interval=0.0001)
    amount = Write.Input("Amount (0=inf) > ", Colors.blue_to_purple, interval=0.0001)
    NThread = 1000

    if Title("Made by Gaming0nDemand"):
        Debug = Write.Input("Debug Fails [y/n] ? > ", Colors.blue_to_purple, interval=0.0001)
